...
def wakefield_from_impedance_fft(
    ...
    This is much faster than `wakefield_from_impedance` for arrays,
    as it evaluates the impedance on a uniform grid, performs an FFT,
    and interpolates to the requested z values.
    ...
    # Wavenumber grid (uniform spacing for FFT)
    # k goes from 0 to k_max
    dk = k_max / (n_fft - 1)
    k_grid = np.linspace(0, k_max, n_fft)

    # Evaluate impedance on grid
    Zk_grid = Zk_func(k_grid)
    ReZ = np.real(Zk_grid)

    # The cosine transform: W(z) = (2c/π) ∫₀^k_max Re[Z(k)] cos(kz) dk
    #
    # We use IRFFT which computes: f_n = (1/N) Σ F_m e^{2πi m n / N}
    # This includes both positive and negative frequencies, effectively
    # computing 2 * ∫₀^k_max F(k) cos(kz) dk when F(k) is real.
    # So we use (c/π) instead of (2c/π) to compensate.

    # z grid from FFT relationship
    # The full FFT needs 2*(n_fft-1) points for the real transform
    n_full = 2 * (n_fft - 1)
    dz = 2 * np.pi / (n_full * dk)
    z_grid = np.arange(n_full) * dz

    # Compute the inverse FFT with proper scaling:
    # IRFFT normalizes by 1/n_full, we need dk for the Riemann sum
    # Factor of (c/π) instead of (2c/π) because irfft includes negative frequencies
    W_grid = irfft(ReZ, n=n_full) * n_full * dk * (c_light / np.pi)

    # The FFT gives us W at z >= 0, but we need it for z <= 0
    # The wake is symmetric: W(-z) = W(z) for z > 0 in this formulation
    # So we can just map |z| to the grid

    # Create interpolator on positive z grid
    interp = interp1d(z_grid, W_grid, kind="cubic", bounds_error=False, fill_value=0.0)

    # Interpolate at |z| (since wake is for z < 0, use -z)
    result = interp(-z)  # -z gives positive values for z < 0

    # Enforce causality: z > 0 should give zero
...
class ResistiveWallImpedance:
    ...
    def wakefield(
        ...
        epsrel : float, optional
            Relative tolerance for integration. Default is 1e-6.
        method : str, optional
            Computation method: 'auto', 'fft', or 'quad'.
            - 'auto': Use FFT for arrays, quad for scalars (default)
            - 'fft': Always use FFT with interpolation (fast)
            - 'quad': Always use numerical quadrature (accurate)
        n_fft : int, optional
            Number of FFT points when using FFT method. Default is 4096.
        ...
        if method == "auto":
            # Use FFT for arrays, quad for scalars
            if is_scalar:
                method = "quad"
            else:
                method = "fft"
