...
def expand_1d_dynamic_fieldmap(z, Ez0, frequency=0, spline_s=0):
    ...
    def Ez(r, z):
        return S(z) - r**2 / 4 * (
            Sp2(z) + (omega / c_light) ** 2 * S(z)
        )  # sin(omega*t)

    def Er(r, z):
        return -r / 2 * Sp(z) + r**3 / 16 * (
            Sp3(z) + (omega / c_light) ** 2 * Sp(z)
        )  # sin(omega*t)

    def Btheta(r, z):
        return (
            (r / 2 * S(z) - r**3 / 16 * (Sp2(z) + (omega / c_light) ** 2 * S(z)))
            * omega
            / c_light**2.0
        )  # cos(omega*t)
...
def expand_radial(r, dfield, frequency=0):
    ...
    if frequency == 0:
        ...
        ftheta = r / 2 * f0 - r**3 / 16 * (
            f2 + ooc2 * f0
        )  # sin(wt) * (-w/c^2 for electric, w for magnetic)

    return fr, fz, ftheta


def expand_fieldmesh_from_onaxis(
    fieldmesh,
    *,
    dr=None,
    nr=10,
    ...
    if method == "fft":
        ...
        if zmirror == "auto":
            if abs(fz[0] / np.abs(fz).max()) > 0.01:
                zmirror = True
        if zmirror:
        ...
        field_theta = (
            field[:, 2, :] * -1j * 2 * np.pi * frequency / c_light**2
        )  # -w/c^2 sin(wt)
        ...
        if frequency == 0:
            ...
            Frf, Fzf, Fthetaf = expand_1d_dynamic_fieldmap(
                zvec, fz, frequency=frequency, spline_s=spline_s
            )
    ...
    if has_Ez:
        ...
        if frequency != 0:
            components["magneticField/theta"] = np.expand_dims(
                field_theta, 1
            )  # -w/c^2 sin(wt)
        ...
        if frequency != 0:
            components["electricField/theta"] = (
                np.expand_dims(field_theta, 1) * 1j * 2 * np.pi * frequency
            )  # w sin(wt)
